...
from util.const.match import OP_TYPE_SEX, SHENHE_CHOICE_LIST, DEFAULT_SEX_INDEX
...
class RequirementService(BaseService):
    ...
    def checkBeforeUpdate(self, opType, valueIndex):
        if opType == OP_TYPE_SEX and self.userInfo.sex == SHENHE_CHOICE_LIST[DEFAULT_SEX_INDEX]:
            return RESP_USER_SEX_FIRST_EDIT
        if opType == OP_TYPE_SEX and int(valueIndex) == SHENHE_CHOICE_LIST.index(self.userInfo.sex):
